---@class vimconfig.util.treesitter
local M = {}
...
function M.get_installed(update)
  if update then
    ...
    local ok, parsers = pcall(require, 'nvim-treesitter.info')
    if ok then
      for _, lang in ipairs(parsers.installed_parsers()) do
...
function M.indentexpr()
  ...
  if ok and indent.get_indent then
...
---@return boolean ok, vimconfig.util.treesitter.Health health
function M.check()
  ...
  ---@class vimconfig.util.treesitter.Health: table<string,boolean>
  local ret = {
...
function M.build(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      VimConfig.error(lines, { title = 'VimConfig Treesitter' })
    end
  end)
end
...
function M.ensure_treesitter_cli(cb)
  ...
  mr.refresh(function()
    ...
    if not p:is_installed() then
      VimConfig.info 'Installing `tree-sitter-cli` with `mason.nvim`...'
      p:install(
        nil,
        vim.schedule_wrap(function(success)
          if success then
            VimConfig.info 'Installed `tree-sitter-cli` with `mason.nvim`.'
            cb(true)
          else
